...
export function addServiceToQuote(id: ServiceId) {
  window.dispatchEvent(new CustomEvent<ServiceId>("pvdm:add-service", { detail: id }));
        ...
        <ul className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s, i) => (
            <Reveal as="li" key={s.id} delay={(i % 3) * 0.07}>
              <article className="group relative flex h-full flex-col overflow-hidden panel p-6 transition-all duration-300 hover:-translate-y-1 hover:border-bolt/50 hover:shadow-lift">
                {/* corner glow on hover */}
                <span
                  aria-hidden
                  className="pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full bg-bolt/20 opacity-0 blur-2xl transition-opacity duration-300 group-hover:opacity-100"
                />
                <div className="flex items-center justify-between">
                  <span className="flex h-12 w-12 items-center justify-center rounded-xl border border-ink-line bg-ink text-bolt transition-colors duration-300 group-hover:border-bolt/50">
                    <ServiceIcon id={s.id} />
                  </span>
                ...
                <button
                  ...
                  className="mt-6 inline-flex items-center gap-1.5 self-start font-mono text-[0.72rem] font-bold uppercase tracking-wider text-bolt transition-transform hover:translate-x-0.5"
                >
                  Add to quote
