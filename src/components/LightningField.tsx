  ...
  useEffect(() => {
    ...
    for (let i = 0; i < boltCount; i++) {
      ...
      bolts.push({
        ...
        baseX,
        z,
        nextStrike: Math.random() * (reduce ? 6 : 2.2),
        life: 0,
        duration: 0,
      });
    }
    ...
    function restrike(b: Bolt) {
      ...
      b.core.geometry = makeTube(pts, 0.02);
      b.glow.geometry = makeTube(pts, 0.1);
      b.duration = 0.3 + Math.random() * 0.32;
      b.life = b.duration;
    }
    ...
    function frame() {
      ...
      for (const b of bolts) {
        ...
        if (b.life <= 0 && b.nextStrike <= 0) {
          restrike(b);
          b.nextStrike = (reduce ? 6 : 2.4) + Math.random() * (reduce ? 6 : 3.4);
        }
        if (b.life > 0) {
          b.life -= dt;
          const k = Math.max(b.life / b.duration, 0);
          // Sharp attack, flickering decay — visible but a touch gentler than a hard strobe.
          const flicker = 0.68 + 0.32 * Math.sin(t * 48 + b.baseX);
          const env = Math.pow(k, 0.55) * flicker;
          (b.core.material as THREE.MeshBasicMaterial).opacity = Math.min(env * 0.95, 0.95);
          (b.glow.material as THREE.MeshBasicMaterial).opacity = env * 0.45;
        } else {
