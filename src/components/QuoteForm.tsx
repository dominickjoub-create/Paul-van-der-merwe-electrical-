...
const valueProps = [
  ...
  "Homes, offices & industrial welcome",
  site.hours.label,
];
...
export function QuoteForm() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("pvdm:add-service", handler);
    return () => window.removeEventListener("pvdm:add-service", handler);
  }, []);
          ...
          <div className="lg:pt-4">
            <Reveal>
              ...
              <h2 className="mt-4 font-display text-4xl font-extrabold leading-[1.02] tracking-tight text-chalk sm:text-5xl">
                Get your quote on{" "}
                <span className="text-bolt">WhatsApp</span>
              </h2>
              ...
              <ul className="mt-8 flex flex-col gap-3">
                {valueProps.map((v) => (
                  <li key={v} className="flex items-center gap-3 text-sm text-chalk">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-bolt/15 text-bolt">
                      ...
                      <button
                        ...
                        className={`rounded-full border px-4 py-2 text-sm font-medium transition-all duration-200 ${
                          on
                            ...
                            : "border-ink-line bg-ink text-chalk-dim hover:border-bolt/50 hover:text-chalk"
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  ...
                  <Field label="Area / Suburb" error={errors.area}>
                    <input
                      ...
                      placeholder="e.g. Weltevreden Park"
                ...
                <div className="mt-4">
                  <Field label="Anything else?" optional>
                    <textarea
                      ...
                      rows={3}
                      placeholder="e.g. power keeps tripping, best time to visit, access is around the back…"
                      className={`${inputCls(false)} resize-none`}
                    />
                  </Field>
                </div>

                <label className="mt-4 flex cursor-pointer items-center gap-3 text-sm text-chalk-dim">
                  <input
                    type="checkbox"
                    checked={applyOffer}
                  ...
                  />
                  Apply my <span className="font-semibold text-bolt">10% first-service</span> discount
                </label>
              </fieldset>

              <button type="submit" className="btn-bolt mt-7 w-full !py-4 text-base">
...
function inputCls(hasError: boolean) {
  return `w-full rounded-lg border bg-ink px-4 py-3 text-[0.95rem] text-chalk placeholder:text-chalk-faint outline-none transition-colors ${
    hasError ? "border-red-400/70" : "border-ink-line focus:border-bolt"
  }`;
}
...
function Step({ n }: { n: number }) {
  return (
    <span className="flex h-6 w-6 items-center justify-center rounded-full bg-bolt font-mono text-xs font-bold text-ink">
      {n}
    </span>
  );
}
